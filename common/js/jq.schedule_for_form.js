...
(function ($) {
    ...
    var methods = {
        ...
        _addScheduleData: function _addScheduleData(timeline, data) {
            var st = Math.ceil((data.start - tableStartTime) / setting.widthTime);
            var et = Math.floor((data.end - tableStartTime) / setting.widthTime);
            var $bar = $('<div class="sc_bar"><span class="head"></span><span class="text"></span><span class="sc-desc"></span><span class="sc-teacher"></span><span class="sc-count"><span></span></span><span class="time"></span></div>');
            ...
            /*if (data.data.maxCount) {
                $bar.find('.sc-count span').text(data.data.maxCount);
            }*/
            ...
            $node.resizable({
                ...
                resize: function resize(ev, ui) {
                    ...
                    var node = $(this);
                    node.width() < 130 ? node.addClass('small') : node.removeClass('small');
                },
                // 要素の移動が終った後の処理
                stop: function stop() {
                    // 130보다 작을 때 제목만 보이도록 클레스 붙여줌
