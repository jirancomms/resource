...
(function ($) {
    ...
    var methods = {
        ...
        _addScheduleData: function _addScheduleData(timeline, data) {
            var st = Math.ceil((data.start - tableStartTime) / setting.widthTime);
            var et = Math.floor((data.end - tableStartTime) / setting.widthTime);
            var $bar = $('<div class="sc_bar"><span class="head"></span><span class="text"></span><span class="sc-teacher"></span><span class="sc-desc"></span><span class="sc-count"></span><span class="time"></span><div class="mouseover"><span class="sc-apply">접수인원 : <span class="sc-apply-count">0</span> / <span class="sc-max-count"></span></span><span class="sc-close">마감</span><span class="sc-select"></span><span class="sc-cancel"></span></div></div>');
            ...
            // 강사
            if (data.data.teacher && data.data.teacher.length > 0) {
                data.data.teacher.map(function(teacher) {
                    var teacherSpan = '<span>' + teacher.split(',')[0] + '</span>';
                    $bar.find('.sc-teacher').append(teacherSpan);
                })
            }

            // 작은 설명
            /*if (data.data.desc) {
                $bar.find('.sc-desc').text(data.data.desc);
            }*/

            // 신청인원
            if (data.data.count) {
                $bar.find('.sc-apply-count').text(data.data.count);
            }

            // 총원
            /*if (data.data.maxCount) {
                $bar.find('.sc-count').text(data.data.maxCount);
            }*/

            // 총원
            if (data.data.maxCount) {
                $bar.find('.sc-max-count').text(data.data.maxCount);
            }
            ...
            // 섹 추가
            if (data.data.color) {
                $($bar).addClass(data.data.color);
            }

            // 고유 클레스 부여
            if (data.data.class) {
                $bar.addClass(data.data.class.toString());
            } // $element.find('.sc_main').append($bar);
            ...
            $bar.bind('mouseup', function () {
                ...
                if (setting.onClick) {
                    if ($(this).data('dragCheck') !== true && $(this).data('resizeCheck') !== true) {
                        ...
                        setting.onClick(node, _scheduleData[scKey], event);
                    }
                }
            });
        ...
        _addRow: function _addRow(timeline, row) {
            ...
            // 130보다 작을 때 제목만 보이도록 클레스 붙여줌
            var node = $(this);
            var scBar = node.find('.sc_bar');
            $.each(scBar, function(i, k) {
                $(k).width() < 130 ? $(k).addClass('small') : $(k).removeClass('small');
            })
        },
